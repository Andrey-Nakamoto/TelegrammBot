...
@bot.message_handler(commands=['euro_to_rub','usd_to_euro','usd_to_rub'])
def quick_rate(message: telebot.types.Message):
    values = []
    if message.text == '/euro_to_rub':
        values = ['euro', 'rub', '1']
...
def convert(message: telebot.types.Message):
    try:
        ...
        if len(values) > 3:
            raise ConversionException('Неправильный ввод. Слишком много параметров ')
        elif len(values) < 2:
            raise ConversionException('Неправильный ввод. Слишком мало параметров ')
        elif len(values) == 2:
            values.append('1')
